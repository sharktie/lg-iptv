...
(function () {
    ...
    function buildEpisodeUrl(episodeId, ext) {
        ...
        return base + '/series/' + encodeURIComponent(cfg.username) + '/' +
               ...
               encodeURIComponent(episodeId) + '.' + (ext || 'mp4');
    }
    ...
    function paintFocus() {
        rows.forEach(function (el, i) { el.classList.toggle('tv-focus-visible', i === focusIdx); });
    ...
    function playEpisode(el) {
        ...
        try { localStorage.setItem('iptv_play_title', epTitle); } catch (e) {}
        window.location.href = '../pages/player.html?url=' + encodeURIComponent(url) +
                               '&title=' + encodeURIComponent(epTitle);
    }

    function activate(el) {
        if (!el) return;
        if (el.id === 'series-back-btn') goBack();
        else playEpisode(el);
    }
    ...
    function render(info) {
        ...
        if (!episodes || typeof episodes !== 'object') {
            if (elMsg) { elMsg.textContent = 'No episodes found.'; elMsg.style.display = 'flex'; }
            return;
        }
        ...
        seasons.forEach(function (sn) {
            ...
            eps.forEach(function (ep) {
                ...
                html += '<button class="episode-row" data-ep="' + escHtml(ep.id) + '"' +
                        ' data-ext="' + escHtml(ext) + '" data-title="' + escHtml(name) + '">' +
                        '<span class="episode-name">' + escHtml(label) + '</span></button>';
            });
        });
        elList.innerHTML = html;

        rows = [document.getElementById('series-back-btn')]
            .concat(Array.prototype.slice.call(elList.querySelectorAll('.episode-row')));
        rows.forEach(function (el, i) {
            el.addEventListener('click', function () { focusIdx = i; paintFocus(); activate(el); });
        });
    ...
    window.addEventListener('keydown', function (e) {
        ...
        if (kc === KEY.UP)    { e.preventDefault(); if (focusIdx > 0) { focusIdx--; paintFocus(); } return; }
        if (kc === KEY.DOWN)  { e.preventDefault(); if (focusIdx < rows.length - 1) { focusIdx++; paintFocus(); } return; }
        if (kc === KEY.ENTER) { e.preventDefault(); activate(rows[focusIdx]); return; }
    }, true);
    ...
    if (!cfg || !cfg.server_url) {
        if (elMsg) { elMsg.textContent = 'No server configured.'; elMsg.style.display = 'flex'; }
        return;
    }
    if (!seriesId) {
        if (elMsg) { elMsg.textContent = 'No series selected.'; elMsg.style.display = 'flex'; }
        return;
    }
    if (elMsg) { elMsg.textContent = 'Loading episodes…'; elMsg.style.display = 'flex'; }

    fetch(apiUrl('action=get_series_info&series_id=' + encodeURIComponent(seriesId)))
        .then(function (r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
        .then(function (data) { render(data); })
        .catch(function () {
            if (elMsg) { elMsg.textContent = 'Could not load episodes.'; elMsg.style.display = 'flex'; }
        });
}());
