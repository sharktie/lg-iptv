...
(function () {
    ...
    function run() {
        ...
        var auth = "username=" + encodeURIComponent(cfg.username) +
                   "&password=" + encodeURIComponent(cfg.password);

        Promise.all([
            fetchJSON(base + "/player_api.php?" + auth + "&action=get_live_streams"),
            fetchJSON(base + "/player_api.php?" + auth + "&action=get_live_categories")
            ...
            save(CHANNEL_CACHE_KEY, { ts: Date.now(), data: slim });
            save(CAT_CACHE_KEY,     { ts: Date.now(), data: categories });
        }).catch(function () {});
    }
