...
(function () {
    ...
    var activeSubLabel = 'off', activeSubSize = 'md';
    ...
    function buildSubsOptions() {
        ...
        subsOptions = [{ kind: 'track', label: 'Off', track: 'off' }];
        tracks.forEach(function (t) { subsOptions.push({ kind: 'track', label: t.label, track: t }); });
        // Text size controls (always available)
        activeSubSize = (window.player && player.getSubSize) ? player.getSubSize() : 'md';
        subsOptions.push({ kind: 'size', label: 'Text: Normal', size: 'md', sep: true });
        subsOptions.push({ kind: 'size', label: 'Text: Large',  size: 'lg' });
        subsOptions.push({ kind: 'size', label: 'Text: Larger', size: 'xl' });
    }
    ...
    function openSubs() {
        ...
        for (var i = 0; i < subsOptions.length; i++) {
            if (subsOptions[i].kind === 'track' && subsOptions[i].label.toLowerCase() === activeSubLabel) { subsIdx = i; break; }
        }
    ...
    function paintSubs() {
        ...
        if (opts[subsIdx]) opts[subsIdx].scrollIntoView({ block: 'nearest' });
    }
    function applySubs() {
