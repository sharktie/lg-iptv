...
(function () {
    ...
    if (window.player) {
        if (typeof player.setPlaybackContext === 'function') {
            player.setPlaybackContext(meta || { title: title || '', name: title || '', searchTitle: title || '', year: meta && meta.year ? meta.year : '' });
        } else if (typeof player.setPlaybackContextFromMeta === 'function') {
            player.setPlaybackContextFromMeta(meta || { title: title || '', name: title || '' });
        }
    }
    ...
    function renderSubsMenu(state) {
        ...
        for (var i = 0; i < subsItems.length; i++) {
            ...
            b.addEventListener('click', function (idx) {
                return function () { chooseSubtitleItem(idx); };
            }(i));
    ...
    function openSubs() {
        ...
        if (window.player && player.openSubtitleBrowser) {
            player.openSubtitleBrowser().then(function (state) { renderSubsMenu(state); });
        } else if (window.player && player.getSubtitleMenuState) {
            renderSubsMenu(player.getSubtitleMenuState());
        } else {
            renderSubsMenu({ title: 'Subtitles', items: [{ kind: 'off', label: 'Off' }] });
        }
    }
    ...
    function moveSubFocus(dir) {
        ...
        for (var i = 0; i < selectable.length; i++) {
            if (selectable[i] === current) { pos = i; break; }
        }
        if (pos < 0) pos = 0;
        pos += dir;
        if (pos < 0) pos = 0;
        ...
        for (var j = 0; j < subsItems.length; j++) {
            if (subsItems[j] === target) { subsIdx = j; break; }
        }
        paintSubs();
    }
    ...
    window.addEventListener('keydown', function (e) {
        ...
        if (subsOpen) {
            e.preventDefault();
            if (kc === KEY.UP) { moveSubFocus(-1); }
            else if (kc === KEY.DOWN) { moveSubFocus(1); }
            else if (kc === KEY.ENTER) { chooseSubtitleItem(subsIdx); }
            else if (kc === KEY.LEFT || kc === KEY.BACK || kc === KEY.ESC) {
                if (!isRootSubtitleState() && window.player && player.closeSubtitleBrowser) {
                    player.closeSubtitleBrowser().then(function (state) { renderSubsMenu(state); });
                } else {
                    closeSubs();
                }
            } else { closeSubs(); }
