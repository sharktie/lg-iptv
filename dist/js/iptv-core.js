...
window.IPTVCore = function () {
  ...
  function base(cfg) {
    return (cfg && cfg.server_url || '').replace(/\/+$/, '');
  }
  function apiUrl(cfg, params) {
    return base(cfg) + '/player_api.php?username=' + encodeURIComponent(cfg.username) + '&password=' + encodeURIComponent(cfg.password) + '&' + params;
  }
  function liveUrl(cfg, id) {
    return base(cfg) + '/live/' + encodeURIComponent(cfg.username) + '/' + encodeURIComponent(cfg.password) + '/' + encodeURIComponent(id) + '.m3u8';
  }
  function movieUrl(cfg, id, ext) {
    return base(cfg) + '/movie/' + encodeURIComponent(cfg.username) + '/' + encodeURIComponent(cfg.password) + '/' + encodeURIComponent(id) + '.' + (ext || 'mp4');
  }
  function episodeUrl(cfg, id, ext) {
    return base(cfg) + '/series/' + encodeURIComponent(cfg.username) + '/' + encodeURIComponent(cfg.password) + '/' + encodeURIComponent(id) + '.' + (ext || 'mp4');
  }
  ...
  function cacheGet(key, ttl) {
    try {
      ...
      if (Date.now() - obj.ts > (ttl || DEFAULT_TTL)) {
