...
function _fetchRemoteManifest() {
  return _updaterFetchWithTimeout(MANIFEST_URL + "?t=" + Date.now()).then(function (res) {
    if (!res.ok) throw new Error("HTTP " + res.status);
    return res.json();
  }).catch(function () {
    return _updaterFetchWithTimeout(MANIFEST_FALLBACK_URL + "?t=" + Date.now()).then(function (res) {
      if (!res.ok) throw new Error("HTTP " + res.status);
      return res.json();
    });
  });
...
function checkForUpdates() {
  _updaterFetchWithTimeout("appinfo.json").then(function (res) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    webOS.service.request("luna://com.webos.appInstallService/dev/install", {
      ...
      onSuccess: function onSuccess(res) {
        if (onProgress) onProgress(90, "Finalising…");
        resolve(res);
      },
      onFailure: function onFailure(err2) {
        reject(new Error(err2 && (err2.errorText || err2.errorCode) ? err2.errorText || String(err2.errorCode) : JSON.stringify(err2)));
      }
    });
  });
}
function _showUpdatePrompt(currentVersion, newVersion, ipkUrl) {
  ...
  overlay.innerHTML = '<div class="modal-box update-modal-box">' + '<div class="update-modal-icon">📺</div>' + '<div class="modal-heading">Update Available</div>' + '<div class="update-modal-versions">' + '<span class="update-ver-old">v' + currentVersion + '</span>' + '<span class="update-ver-arrow">→</span>' + '<span class="update-ver-new">v' + newVersion + '</span>' + '</div>' + '<p class="update-modal-desc">A new version is ready to install. The app will restart automatically after updating.</p>' + '<div class="modal-btns update-modal-btns">' + '<button class="modal-btn" id="update-later-btn">Later</button>' + '<button class="modal-btn modal-btn-ok" id="update-now-btn">⬇ Update Now</button>' + '</div>' + '<div id="update-progress" class="update-progress" style="display:none">' + '<div class="update-progress-bar"><div class="update-progress-fill" id="update-progress-fill"></div></div>' + '<div class="update-progress-msg" id="update-progress-msg">Installing…</div>' + '</div>' + '</div>';
  ...
  nowBtn.addEventListener("click", function () {
    ...
    _installUpdate(ipkUrl, function (pct, msg) {
      _setUpdateProgress(pct, msg);
    }).then(function () {
