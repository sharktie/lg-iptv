...
(function () {
  ...
  /* ── EPG helpers ─────────────────────────────────────────────────── */
  function epgStart(e) {
    return Number(e.start_timestamp || 0) * 1000;
  }
  function epgEnd(e) {
    return Number(e.stop_timestamp || e.end_timestamp || 0) * 1000;
  }
  function decode(s) {
    return typeof xtreamDecodeEPG === 'function' ? xtreamDecodeEPG(s) : s || '';
  }
  function fmtTime(ms) {
  ...
  function whenLabel(e) {
    return dayLabel(epgStart(e)) + ' · ' + fmtTime(epgStart(e));
  }
  function archiveDays(ch) {
    return parseInt(ch.tv_archive_duration, 10) || 0;
  }
  ...
  favs.forEach(function (id) {
    favSet[String(id)] = true;
  });
  function isFav(id) {
    return !!favSet[String(id)];
  }
  ...
  function slotKey(hour, minute) {
    return pad2(hour) + ':' + pad2(minute);
  }
  function pinId(streamId, hour, minute) {
    return String(streamId) + '@' + slotKey(hour, minute);
  }
  ...
  function addPin(p) {
    for (var i = 0; i < pins.length; i++) if (pins[i].id === p.id) return;
    pins.push(p);
    save('catchup_pins', pins);
  }
  ...
  function buildCatMaps(catsOrdered) {
    ...
    Object.keys(byCat).forEach(function (id) {
      if (!seen[id]) {
        catOrder.push(id);
        seen[id] = true;
        if (!catName[id]) catName[id] = 'Other';
      }
    });
  }
  function loadChannels() {
    ...
    var cachedCh = IPTVCore.cacheGet(chKey),
      cachedCat = IPTVCore.cacheGet(catKey);
    if (cachedCh && cachedCh.length) {
      allChannels = cachedCh;
      catName = {};
      (cachedCat || []).forEach(function (c) {
        catName[String(c.category_id)] = c.category_name || 'Unnamed';
      });
    ...
    Promise.all([xtreamGetCategories(cfg).catch(function () {
      return [];
    }), xtreamGetLiveChannels(cfg)]).then(function (res) {
      var cats = res[0] || [],
        list = res[1] || [];
      var arch = [];
  ...
  function makeChRow(ch) {
    ...
    row.className = 'cu-ch-row';
    row.tabIndex = -1;
    row.dataset.sid = String(ch.stream_id);
    ...
    if (ch.stream_icon) {
      ...
      img.onerror = function () {
        ...
        fb.className = 'cu-ch-logo-fallback';
    ...
    if (days) {
      ...
      badge.className = 'cu-ch-badge';
      badge.textContent = days + 'd';
      row.appendChild(badge);
    }
    row.appendChild(fav);
    row.addEventListener('click', function () {
      pendingMainFocus = true;
      selectChannel(ch);
    });
    row._ch = ch;
    row._fav = fav;
  ...
  function makeSection(id, label, channels) {
    ...
    sec.className = 'cu-section';
    sec.dataset.secId = id;
    var hdr = document.createElement('button');
    hdr.className = 'cu-section-hdr';
    hdr.innerHTML = '<span class="cu-sec-chev">▾</span><span class="cu-sec-label">' + escHtml(label) + '</span><span class="cu-sec-count">' + channels.length + '</span>';
    var list = document.createElement('div');
    list.className = 'cu-section-list';
    sec.appendChild(hdr);
    sec.appendChild(list);
    sec._channels = channels;
    sec._built = false;
    function build() {
      if (sec._built) return;
      var frag = document.createDocumentFragment();
      channels.forEach(function (ch) {
        frag.appendChild(makeChRow(ch));
      });
  ...
  function renderSidebar() {
    ...
    if (pins.length) {
      ...
      cw.innerHTML = '<span class="cu-cw-ico">▶</span><span class="cu-sec-label">Continue Watching</span>' + '<span class="cu-sec-count">' + pins.length + '</span>';
    ...
    var favChannels = allChannels.filter(function (ch) {
      return isFav(ch.stream_id);
    });
  ...
  function doToggleFav(sid) {
    ...
    if (zone === 'sidebar') {
      ...
      for (var i = 0; i < items.length; i++) {
        if (items[i].kind === 'channel' && String(items[i].ch.stream_id) === String(sid)) {
          idx = i;
          break;
        }
      }
  ...
  function openHome() {
    mainMode = 'cw';
    currentCh = null;
    elArchive.hidden = true;
    elCw.hidden = false;
    elMainTitle.textContent = 'Continue Watching';
    document.querySelectorAll('#cu-nav .cu-ch-row.selected').forEach(function (r) {
      r.classList.remove('selected');
    });
    document.querySelectorAll('#cu-nav .cu-cw-nav').forEach(function (n) {
    ...
    pins.forEach(function (p) {
      ...
      body.innerHTML = '<div class="cu-cw-title">' + escHtml(p.label || 'Programme') + '</div>' + '<div class="cu-cw-channel">' + escHtml((p.channel_name || '') + ' · ' + slotLabel(p)) + '</div>' + '<div class="cu-cw-when">Loading…</div>';
      var play = document.createElement('div');
      play.className = 'cu-cw-play';
      play.textContent = '▶';
      var unpin = document.createElement('button');
      unpin.className = 'cu-cw-unpin';
      unpin.textContent = '★';
      unpin.tabIndex = -1;
      unpin.title = 'Unpin';
      ...
      card.addEventListener('click', function () {
        if (card._airing) playListing(p.stream_id, p.channel_name, p.channel_icon, card._airing);
      });
      ...
      getDataTable(p.stream_id).then(function (listings) {
        ...
        if (airing) {
          ...
          when.textContent = whenLabel(airing);
          card.classList.remove('cu-cw-empty');
        } else {
          when.textContent = 'No recent airing';
  ...
  function resolvePin(p, listings) {
    var now = Date.now(),
      winStart = now - 14 * 86400000,
      slot = p.hour * 60 + p.minute,
      TOL = 45;
    var best = null,
      bestStart = 0;
  ...
  function doUnpin(id) {
    ...
    if (zone === 'main') {
      cwIndex = 0;
      cwSub = 'play';
      if (pins.length) paintCw();else focusSidebar();
    }
  }
  ...
  function selectChannel(ch) {
    ...
    getDataTable(ch.stream_id).then(function (listings) {
      ...
      if (!daysData.length) {
        showPlaceholder('No catch-up recordings available for this channel.');
        return;
      }
      hidePlaceholder();
      renderDayTabs();
  ...
  function buildDays(listings, days) {
    var now = Date.now(),
      winStart = days ? now - days * 86400000 : 0,
      groups = {};
    for (var i = 0; i < listings.length; i++) {
      var e = listings[i],
        s = epgStart(e),
  ...
  function renderProgrammes() {
    ...
    day.items.forEach(function (e) {
      ...
      var row = document.createElement('div');
      row.className = 'cu-prog-row';
      row.tabIndex = -1;
      row.innerHTML = '<span class="cu-prog-time">' + escHtml(fmtTime(s) + ' – ' + fmtTime(en)) + '</span>' + '<span class="cu-prog-title">' + escHtml(decode(e.title) || 'Unknown programme') + '</span>' + '<button class="cu-prog-pin' + (pinned ? ' active' : '') + '" tabindex="-1" title="Pin to Continue Watching">★</button>' + '<span class="cu-prog-play">▶</span>';
      row._listing = e;
      row.querySelector('.cu-prog-pin').addEventListener('click', function (ev) {
        ev.stopPropagation();
        doTogglePin(e, row);
      });
      row.addEventListener('click', function () {
        playListing(currentCh.stream_id, currentCh.name, currentCh.stream_icon, e);
      });
  ...
  function doTogglePin(e, row) {
    if (!currentCh) return;
    var dt = new Date(epgStart(e)),
      h = dt.getHours(),
      m = dt.getMinutes();
    var id = pinId(currentCh.stream_id, h, m),
      prevLen = pins.length;
    if (isPinnedSlot(currentCh.stream_id, h, m)) removePin(id);else addPin({
      id: id,
      stream_id: currentCh.stream_id,
      channel_name: currentCh.name,
      channel_icon: currentCh.stream_icon,
      hour: h,
      minute: m,
      label: decode(e.title)
    });
  ...
  function playListing(streamId, chName, chIcon, e) {
    ...
    window.location.href = '../pages/player.html?url=' + encodeURIComponent(urls[0]) + '&title=' + encodeURIComponent(title);
  }
  ...
  var currentCh = null;
  var sideIndex = 0,
    sideSub = 'row'; // sideSub: 'row' | 'star'
  var cwIndex = 0,
    cwSub = 'play'; // cwSub: 'play' | 'unpin'
  var archSub = 'days'; // 'days' | 'progs'
  var dayIndex = 0,
    progIndex = 0,
    progSub = 'play'; // progSub: 'play' | 'pin'
  ...
  function clearRings() {
    ...
    document.querySelectorAll('.cu-row-active').forEach(function (el) {
      el.classList.remove('cu-row-active');
    });
  }
  function visible(el) {
  ...
  function sideItems() {
    ...
    return els.map(function (el) {
      if (el.classList.contains('cu-cw-nav')) return {
        kind: 'cw',
        el: el
      };
      if (el.classList.contains('cu-section-hdr')) return {
        kind: 'header',
        el: el
      };
      return {
  ...
  function cwCards() {
    return Array.prototype.slice.call(elCw.querySelectorAll('.cu-cw-card'));
  }
  function dayTabs() {
    return Array.prototype.slice.call(elDayTabs.querySelectorAll('.cu-day-tab'));
  }
  function progRows() {
    return Array.prototype.slice.call(elProgList.querySelectorAll('.cu-prog-row'));
  }
  function paintSidebar() {
    ...
    if (it.kind === 'channel' && sideSub === 'star') {
      it.favBtn.classList.add('tv-focus-visible');
      it.el.classList.add('cu-row-active');
  ...
  function paintCw() {
    ...
    var c = cards[cwIndex];
    if (cwSub === 'unpin') {
      c.querySelector('.cu-cw-unpin').classList.add('tv-focus-visible');
      c.classList.add('cu-row-active');
  ...
  function paintArchive() {
    ...
    if (progSub === 'pin') {
      r.querySelector('.cu-prog-pin').classList.add('tv-focus-visible');
      r.classList.add('cu-row-active');
  ...
  function focusMain() {
    if (mainMode === 'cw') {
      ...
      zone = 'main';
      cwIndex = clamp(cwIndex, 0, cwCards().length - 1);
      cwSub = 'play';
      paintCw();
      return true;
    }
    ...
    zone = 'main';
    archSub = dayTabs().length ? 'days' : 'progs';
    progSub = 'play';
    paintArchive();
    return true;
  }
  function onSidebarKey(kc) {
    ...
    if (kc === KEY.UP) {
      ...
      sideSub = 'row';
      if (sideIndex < items.length - 1) {
        sideIndex++;
        paintSidebar();
      }
    } else if (kc === KEY.RIGHT) {
      if (it && it.kind === 'channel' && sideSub === 'row') {
        sideSub = 'star';
        paintSidebar();
      } else {
        focusMain();
      ...
      if (it && it.kind === 'channel' && sideSub === 'star') {
        sideSub = 'row';
        paintSidebar();
      }
  ...
  function onMainCwKey(kc) {
    ...
    if (kc === KEY.UP) {
      ...
      var c = cards[cwIndex];
      if (!c) return;
      if (cwSub === 'unpin') doUnpin(c.dataset.pinId);else c.click();
    }
  }
  function onMainArchiveKey(kc) {
    if (archSub === 'days') {
      var tabs = dayTabs();
      if (kc === KEY.LEFT) {
        if (dayIndex === 0) focusSidebar();else {
          dayIndex--;
          paintArchive();
        }
      } else if (kc === KEY.RIGHT) {
        if (dayIndex < tabs.length - 1) {
          dayIndex++;
          paintArchive();
        }
      } else if (kc === KEY.DOWN) {
    ...
    if (kc === KEY.UP) {
      ...
      var r = rows[progIndex];
      if (!r) return;
      if (progSub === 'pin') doTogglePin(r._listing, r);else r.click();
    }
  }
  window.addEventListener('keydown', function (e) {
    ...
    if (zone === 'main') {
      if (mainMode === 'cw') onMainCwKey(kc);else onMainArchiveKey(kc);
      return;
    }
  }, true);

  /* ── Header + search wiring ──────────────────────────────────────── */
  document.getElementById('cu-home-btn').addEventListener('click', function () {
    tvGoBack('../index.html');
  });
  document.getElementById('cu-settings-btn').addEventListener('click', function () {
  ...
  if (!cfg) {
    setStatus('No server');
    showPlaceholder('No server configured — open Settings first.');
    return;
  }
  if (cfg.type === 'm3u') {
